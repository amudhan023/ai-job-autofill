...
| T6 | **Backend-unreachable UX**: audit popup/options flows for a down or misconfigured backend; ensure clear non-blocking messaging and that deterministic fill is never degraded. Add retry/backoff to `extension/src/api/` client if missing. | — | in-progress | Verify current behavior first — parts may already exist. |
