...
| T2 | **Release packaging**: `npm run package` script that builds and zips `extension/dist` with a version stamp for Chrome Web Store upload; CI uploads the zip as an artifact on main. | T1 | in-progress | Keep manifest/package versions in sync (single source). |
