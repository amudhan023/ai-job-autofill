...
| T8 | **Perf benchmarks**: scan/fill timing over the synthetic corpus (`content/corpus.test.ts` fixtures); assert a generous budget in a vitest bench/test so regressions surface in CI. | T1 | in-progress | Planned in M6 (ARCHITECTURE_REVIEW §4); corpus landed, benchmarks did not. |
