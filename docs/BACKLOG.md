...
| T3 | **Coverage reporting in CI**: run `vitest --coverage` and `pytest --cov` in CI, publish summaries; add a modest fail-under floor (e.g. 70%) to prevent regression, not to chase 100%. | T1 | in-progress | Thresholds must pass on current code as-is. |
