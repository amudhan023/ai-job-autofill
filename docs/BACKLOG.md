...
|----|------|-----------|--------|-------|
| T1 | **Lint gates**: ESLint (flat config, typescript-eslint) for `extension/` with `npm run lint`; ruff (`ruff check`) for `backend/`; both added as CI steps. Fix (or narrowly suppress with justification) existing violations. No repo-wide reformat — formatting policy is T9. | — | ready | Foundation for all later autonomous merges. |
